...
//! | `records_total`  | u64    | Total GPS records published (accumulated)|
...
/// Called once the device IMEI is confirmed after the TCP handshake.
///
/// Sets `status=connected`, records the peer address and connection timestamp,
/// and increments the lifetime session counter.
...
/// Called on every successfully parsed AVL packet from the device.
///
/// Updates `last_seen_at` and increments `packets_total` and `records_total`.
...
///
/// Increments `commands_total` by `count`.
...
///
/// Sets `status=disconnected`, records `disconnected_at`, and computes
/// `last_session_ms` from the connection timestamp.
